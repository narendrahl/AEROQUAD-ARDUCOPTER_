...
void measureMagnetometer(float roll, float pitch) {
  ...
  const float magY = (float)measuredMagY * cosRoll -
					 ...
					 */
					 ...
					 // Added on 24-04 for compensate tilt
  
  const float magX = (float)measuredMagX * cosRoll +
                     (float)measuredMagZ * sinRoll; 
  const float magY = (float)measuredMagY * cosPitch -
                     (float)measuredMagZ * sinPitch;
  //const float magX = (float)measuredMagX;
  //const float magY = (float)measuredMagY;
  const float tmp  = sqrt(magX * magX + magY * magY);
   
  hdgX = magX / tmp;
  hdgY = magY / tmp;
  
  filter_hdgX[(filter_hdgX_Index%5)]=hdgX;
  filter_hdgY[filter_hdgY_Index%5]=hdgY;
  for(int i=0;i<5;i++)
  {
   hdgX+=filter_hdgX[i];
   hdgY+=filter_hdgY[i];
  ...
  if(fmod(filter_hdgX_Index,5)==0||fmod(filter_hdgY_Index,5)==0)
   { filter_hdgX_Index=0;
     filter_hdgY_Index=0;}
  else
   {  filter_hdgX_Index++;
